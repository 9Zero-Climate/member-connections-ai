...
export const getAssistant = (config: Config): Assistant => {
    ...
    userMessage: async ({ message: slackMessage, say, setTitle, client }) => {
      ...
      try {
        ...
        while (remainingLlmLoopsAllowed > 0) {
          ...
          const streamFromLlm = await openRouter.chat.completions.create({
            // Using Gemini 2.0 Flash for speed, context window and quality
            model: 'google/gemini-2.0-flash-001',
            messages: llmThread,
            tools: tools as ChatCompletionTool[],
            tool_choice: remainingLlmLoopsAllowed > 0 ? 'auto' : 'none',
            stream: true,
            max_tokens: config.maxMessageLength / 4,
          });
